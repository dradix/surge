...
// Returns rook attacks from a given square, using the Hyperbola Quintessence Algorithm. Only used
// to initialize the magic lookup table
...
// Returns the 'x-ray attacks' for a rook at a given square. X-ray attacks cover squares that are
// not immediately accessible by the rook, but become available when the immediate blockers are
...
// Returns bishop attacks from a given square, using the Hyperbola Quintessence Algorithm. Only used
// to initialize the magic lookup table
...
void initialise_bishop_attacks()
{
  ...
  for (Square sq = a1; sq <= h8; ++sq)
  {
    ...
    BISHOP_ATTACK_MASKS[sq] =
        (MASK_DIAGONAL[diagonal_of(sq)] ^ MASK_ANTI_DIAGONAL[anti_diagonal_of(sq)]) & ~edges;
...
// Returns the 'x-ray attacks' for a bishop at a given square. X-ray attacks cover squares that are
// not immediately accessible by the rook, but become available when the immediate blockers are
// removed from the board
...
// Initializes the lookup table for the bitboard of all squares along the line of two given squares
// (0 if the two squares are not aligned)
...
// Initializes the table containg pseudolegal attacks of each piece for each square. This does not
// include blockers for sliding pieces
...
// Initializes lookup tables for rook moves, bishop moves, in-between squares, aligned squares and
// pseudolegal moves
